...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchJackpotData = async () => {
      try {
        // Initial API call to fetch jackpot data on page load
        ...
        console.log(participants[0].user);
        console.log(participants[0].items);
        ...
        const initialItems = participants.map((participant: any, index: any) => {
          ...
          const totalValue = participant.items.reduce((acc: any, item: any) => acc + item.price, 0); // Calculate total price (assuming item has price field)
    ...
    socket.on('jackpots', async (data) => {
      if (data.action === 'update') {
        // Fetch the updated jackpot data
        const response = await axios.get('http://localhost:5000/jackpotSystem/status');
        const participants = response.data.participants; // Access participants array
          ...
          <div className="w-full">
            <div className="w-[95%] flex flex-wrap justify-center md:justify-start gap-2 mt-7 mx-auto">
              {items.map((item :any) => (
                <div
                  key={item.id}
                  className={`w-[250px] border-[1px] border-black py-1 px-2`}
                  style={{
