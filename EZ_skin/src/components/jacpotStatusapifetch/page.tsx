...
export default function JackpotStatus() {
  const [participants, setParticipants] = useState([]); // Initialize state for participants
  ...
  useEffect(() => {
    const fetchJackpotData = async () => {
      try {
        ...
        // Map participants to a structure suitable for the wheel
        const initialParticipants = participants.map((participant: any) => {
          ...
          return {
            ...
            totalValue: totalValue, // Total price of all items contributed by the user
            skinCount: participant.items.length, // Number of skins contributed
            img: user.avatar.small || "path-to-image", // Use user's image if available
          };
        });

        // Update state with the initial fetched participants
    ...
    socket.on('jackpots', async (data) => {
      if (data.action === 'update') {
        ...
        // Map participants to the correct structure
        ...
        // Update state with the new participants
      ...
      {showWheel && (
        <div className="w-full mt-10 text-center">
          ...
          <div className="w-full mt-5">
            <HorizontalWheel participants={participants} /> {/* Pass participants data to wheel */}
          </div>
        </div>
      )}
...
//           </h2>
//           <div className="w-full mt-5">
//             <HorizontalWheel participants={items} /> {/* Pass the participants data */}
//           </div>
//         </div>
//       )}
...
//               className={`w-[250px] border-[1px] border-black py-1 px-2 bg-black`}
//               style={{
//                 backgroundColor: item.backgroundColor || "#222",
//                 borderRight: `20px solid ${item.borderColor || "#444"}`,
//               }}
//             >
//               <div className="flex items-center gap-1">
//                 <Image
